...
void execve_(const char *file, char *argv[], char *envp[]) {
  ...
  assert(fd > 0);
  Elf64_Ehdr *h = (Elf64_Ehdr *)mmap(NULL, 4096, PROT_READ, MAP_PRIVATE, fd, 0);
  assert(h != (void *)-1);
  ...
  for (int i = 0; i < h->e_phnum; i++) {
    ...
    if (p->p_type == PT_LOAD) {
      ...
      if (p->p_flags & PF_X)
        prot |= PROT_EXEC;
      void *ret = mmap(ROUND(p->p_vaddr, p->p_align), // addr, rounded to ALIGN
  ...
  asm volatile("mov $0, %%rdx;" // required by ABI
               "mov %0, %%rsp;"
               "jmp *%1"
               :
